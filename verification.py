...
# %%
# 3.3b — Explicit re-verification: confirm the PCA-reduced arrays still carry
# zero subject overlap. X_tr_pca/X_val_pca are derived from X_tr/X_val (which
# were already split by GroupShuffleSplit on subject), and pca_95 was fit on
# X_tr only — so no leakage should have been reintroduced. Checking directly
# rather than assuming it carried over silently.
tr_idx_check = X_tr.index
val_idx_check = X_val.index

tr_subjects_check = set(train.loc[tr_idx_check, "subject"])
val_subjects_check = set(train.loc[val_idx_check, "subject"])

overlap = tr_subjects_check & val_subjects_check
assert len(overlap) == 0, f"LEAKAGE DETECTED: subjects {overlap} appear in both splits"
print("Verified: zero subject overlap between X_tr_pca source rows and X_val_pca source rows.")
print(f"Train subjects ({len(tr_subjects_check)}):", sorted(tr_subjects_check))
print(f"Val subjects ({len(val_subjects_check)}):", sorted(val_subjects_check))

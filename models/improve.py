...
def train_model(hidden_sizes, dropout=0.0, batchnorm=False, weight_decay=0.0,
                 lr=1e-3, batch_size=64, optimizer_name="adam",
                 max_epochs=60, patience=8, seed=42, verbose=False):
...
# %% [markdown]
# ## 6.4 — Learning rate investigation
# Too low -> slow convergence, may not reach a good optimum within max_epochs.
# Too high -> unstable training, can overshoot good solutions or diverge.
...
for lr in [1e-4, 3e-4, 1e-3, 3e-3, 1e-2]:
    mean_f1, std_f1 = eval_config_multiseed(best_hidden_sizes, dropout=best_reg["dropout"],
                                              ...
                                              batchnorm=best_batchnorm, lr=lr)
    lr_results.append({"lr": lr, "val_macro_f1_mean": mean_f1, "val_macro_f1_std": std_f1})
    print(f"lr={lr:<8} | val Macro F1 = {mean_f1:.4f} ± {std_f1:.4f}")

lr_df = pd.DataFrame(lr_results).sort_values("val_macro_f1_mean", ascending=False)
best_lr = float(lr_df.iloc[0]["lr"])
print(f"\nBest learning rate: {best_lr}")

# %% [markdown]
# ## 6.5 — Batch size investigation
# Smaller batches -> noisier gradient estimates (can help escape poor local
# minima, but slower per epoch). Larger batches -> smoother, faster per-epoch
# updates, but can generalize slightly worse and need more epochs to converge.
...
for bs in [16, 32, 64, 128, 256]:
    mean_f1, std_f1 = eval_config_multiseed(best_hidden_sizes, dropout=best_reg["dropout"],
                                              weight_decay=best_reg["weight_decay"],
                                              batchnorm=best_batchnorm, lr=best_lr, batch_size=bs)
    batch_results.append({"batch_size": bs, "val_macro_f1_mean": mean_f1, "val_macro_f1_std": std_f1})
    print(f"batch_size={bs:<4} | val Macro F1 = {mean_f1:.4f} ± {std_f1:.4f}")

batch_df = pd.DataFrame(batch_results).sort_values("val_macro_f1_mean", ascending=False)
best_batch_size = int(batch_df.iloc[0]["batch_size"])
print(f"\nBest batch size: {best_batch_size}")

# %% [markdown]
# ## 6.6 — Optimizer investigation
# Adam adapts the learning rate per-parameter using running gradient
# statistics -- generally a strong default. SGD+momentum is simpler, can
# generalize better in some cases but is more sensitive to the learning rate
# choice. RMSprop is similar to Adam without the momentum term.

# %%
opt_results = []
for opt_name in ["adam", "sgd_momentum", "rmsprop"]:
    mean_f1, std_f1 = eval_config_multiseed(best_hidden_sizes, dropout=best_reg["dropout"],
                                              weight_decay=best_reg["weight_decay"],
                                              batchnorm=best_batchnorm, lr=best_lr,
                                              batch_size=best_batch_size, optimizer_name=opt_name)
    opt_results.append({"optimizer": opt_name, "val_macro_f1_mean": mean_f1, "val_macro_f1_std": std_f1})
    print(f"optimizer={opt_name:<13} | val Macro F1 = {mean_f1:.4f} ± {std_f1:.4f}")

opt_df = pd.DataFrame(opt_results).sort_values("val_macro_f1_mean", ascending=False)
best_optimizer = opt_df.iloc[0]["optimizer"]
...
# %% [markdown]
# ## 6.7 — Final model: all winning hyperparameters combined
...
for s in final_seeds:
    model, val_f1, hist = train_model(
        ...
        batchnorm=best_batchnorm, lr=best_lr, batch_size=best_batch_size,
        optimizer_name=best_optimizer, max_epochs=100, patience=12, seed=s,
    )
...
print(f"\nFinal NN config: hidden_sizes={best_hidden_sizes}, dropout={best_reg['dropout']}, "
      ...
      f"lr={best_lr}, batch_size={best_batch_size}, optimizer={best_optimizer}")
...
with open("processed/nn_results.pkl", "wb") as f:
    pickle.dump({
        ...
        "norm_sweep": norm_df,
        "lr_sweep": lr_df,
        "batch_sweep": batch_df,
        "optimizer_sweep": opt_df,
